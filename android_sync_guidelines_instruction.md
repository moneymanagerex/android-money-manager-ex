...
3. **Applying Received Records:**
  ...
  * **If the remote record ALREADY exists locally (same `pb_id`):**
    ...
    * Otherwise, update all local fields with the remote values (mapping `_updated_at` to `pb_updated_at`), setting `pb_is_dirty = 2` during the update and resetting it to `0` once the transaction is finalized.
