...
public class PreferencesActivity extends BaseFragmentActivity {
    ...
    public static class PreferencesFragment extends PreferenceFragment {
        ...
        public void onCreate(Bundle savedInstanceState) {
            ...
            try {
                getActivity().setTheme(mCore.getThemeApplication());
            } catch (Exception e) {
                Log.e(LOGCAT, e.getMessage());
            }

            super.onCreate(savedInstanceState);
            PreferenceManager.setDefaultValues(getActivity(), R.xml.prefrences, false);
            // set layout
                ...
                if (preference instanceof PreferenceScreen)
                    if (((PreferenceScreen) preference).getDialog() != null)
                        ((PreferenceScreen) preference).getDialog().getWindow().getDecorView()
                                .setBackgroundDrawable(getActivity().getWindow().getDecorView().getBackground().getConstantState().newDrawable());
            return false;
        }
        ...
        private void showWebTipsDialog(final String key, final CharSequence title, final int rawResources, boolean force) {
            if (!force) {
                if (getActivity().getSharedPreferences(TipsDialogFragment.PREF_DIALOG, 0).getBoolean(key, false)) return;
            }
